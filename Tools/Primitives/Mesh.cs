...
namespace Tools.Primitives
{
	public class Mesh : Primitive
	{
		...
		public void texturing(Scene.Camera camera, Bitmap texture, float[,] zBuffer, Color[,] frameBuffer)
		{
			foreach (var lst in Faces)
			{
				var polygon = new Triangle3D(Vertexes[lst[0]], Vertexes[lst[1]], Vertexes[lst[2]]);
				...
				ConvertToRasterWithTexture(p1, p2, p3, textureVertex[0], textureVertex[1], textureVertex[2], texture, zBuffer, frameBuffer);
			}

		}

		private void ConvertToRasterWithTexture(Point3D p0, Point3D p1, Point3D p2, PointF tp0, PointF tp1, PointF tp2, Bitmap texture, float[,] zBuffer, Color[,] frameBuffer)
		{
			if (p1.Y < p0.Y)
			{
				var temp = p0;
			...
			var z01 = InterpolateListFloat((int)p0.Y, p0.Z, (int)p1.Y, p1.Z);
			var z12 = InterpolateListFloat((int)p1.Y, p1.Z, (int)p2.Y, p2.Z);
			var z02 = InterpolateListFloat((int)p0.Y, p0.Z, (int)p2.Y, p2.Z);
			...
			for (int y = (int)p0.Y; y < (int)p2.Y; y++)
			{
				...
				var z_segment = InterpolateListFloat(x_l, z_left[(int)(y - p0.Y)], x_r, z_right[(int)(y - p0.Y)]);
		...
		private List<float> InterpolateListFloat(int x1, float y1, int x2, float y2)
		{
